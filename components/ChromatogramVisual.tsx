  ...
  return (
    <div className={`relative overflow-hidden rounded-4xl border border-white/10 bg-midnight/70 p-5 shadow-glass ${className}`} aria-label="Abstract chromatogram line visualization">
      ...
      <div className="relative mt-4 grid gap-3 sm:grid-cols-3">
        {['Batch traceability', 'COA review', 'HPLC / LCMS files'].map((label) => <div key={label} className="rounded-2xl border border-white/10 bg-white/10 px-4 py-3 text-xs font-semibold uppercase tracking-[0.18em] text-white/70">{label}</div>)}
      </div>
    </div>
  );
}
