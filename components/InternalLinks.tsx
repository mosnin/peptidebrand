  ...
  return (
    <nav aria-label="Recommended internal links" className="rounded-3xl bg-mist p-6">
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        {links.map((link) => (
          <Link key={link.href} href={link.href} className="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 transition hover:border-teal hover:text-teal">
            {link.label}
          </Link>
        ))}
      </div>
