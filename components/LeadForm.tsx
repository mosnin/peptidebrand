...
export function LeadForm({ variant = 'quote' }: { variant?: LeadFormVariant }) {
  const id = useId();
  ...
  return (
    <form name={config.formName} onSubmit={handleSubmit} className="grid gap-5 rounded-4xl border border-slate-200 bg-white p-6 shadow-soft" noValidate>
      ...
      <div>
        <h2 className="font-display text-3xl font-bold text-navy">{config.title}</h2>
      ...
      {status === 'loading' && <p role="status" className="text-sm font-semibold text-slate-600">Submitting securely…</p>}
      {status === 'success' && <p role="status" className="rounded-2xl bg-blue/10 px-4 py-3 text-sm font-semibold text-blue">Submission received. Redirecting to the confirmation page…</p>}
      <button disabled={status === 'loading'} className="rounded-full border border-blue bg-blue px-5 py-3 font-black text-white shadow-glow transition hover:bg-navy disabled:cursor-not-allowed disabled:opacity-70">{status === 'loading' ? 'Submitting…' : config.submitLabel}</button>
      <p className="text-xs text-slate-500">Privacy note: your details are used to respond to this research-use business inquiry.</p>
    </form>
  );
}
