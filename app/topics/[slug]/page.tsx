      ...
      <Section className="border-b border-slate-200 bg-mist">
        <Container>
          <div className="max-w-4xl">
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-blue">Sourcing guide</p>
            <h1 className="mt-4 text-4xl font-black tracking-tight text-navy sm:text-6xl">{page.h1}</h1>
            <p className="mt-6 text-xl leading-8 text-slate-600">{page.intro}</p>
            <div className="mt-8 rounded-3xl border border-blue/20 bg-white p-5 text-sm leading-6 text-slate-700">
              <strong className="block text-navy">Research-use-only disclaimer</strong>
              {topicDisclaimer}
            </div>
        ...
        <Container>
          <div className="grid gap-8 lg:grid-cols-[2fr_1fr]">
            <div className="space-y-8">
              <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-soft">
                <h2 className="text-2xl font-bold text-navy">Who this guide is for</h2>
                <p className="mt-3 text-slate-600">{page.audience}</p>
                <p className="mt-4 text-slate-600">
                  Use this guide to brief supplier conversations, organize batch documentation, prepare label copy, and connect to the right service team for deeper sourcing help.
                </p>
              </section>
            ...
            <aside className="space-y-6">
              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-soft">
                ...
                <Link href={page.primaryService.href} className="mt-4 inline-flex font-semibold text-blue">{page.primaryService.label} →</Link>
              </div>
              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-soft">
                <h2 className="text-xl font-bold text-navy">Related pages</h2>
                <div className="mt-4 grid gap-3">
                  {page.related.map((item) => (
                    <Link key={item.href} href={item.href} className="rounded-2xl bg-slate-50 p-3 text-sm font-semibold text-navy hover:text-blue">{item.label}</Link>
                  ))}
                </div>
              </div>
            </aside>
      ...
      <Section className="bg-slate-50">
        <Container>
          <div className="grid gap-10 lg:grid-cols-[1fr_1.2fr]">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.25em] text-blue">FAQ</p>
              <h2 className="mt-3 text-3xl font-black text-navy">Questions teams ask before acting on this guide</h2>
